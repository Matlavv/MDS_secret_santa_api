...
describe('createGroup', () => {
    it('should create a new group', async () => {
        ...
        Group.create = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Test Group', users: ['user1'] });
        ...
        expect(res.json).toHaveBeenCalledWith({ message: 'Group created id : group1' });
    });

    it('should not create a group if the name already exists', async () => {
        Group.findOne = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Test Group', users: ['user1'] });
        ...
        expect(res.json).toHaveBeenCalledWith({ message: 'Group name must be unique' });
    });
    describe('getInfoGroup', () => {
        it('should return group information', async () => {
            Group.findById = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Test Group', users: ['user1'] });
    ...
    describe('deleteGroup', () => {
        it('should delete a group', async () => {
            User.findById = jest.fn().mockResolvedValue({ _id: 'user1' });
            Group.findById = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Test Group', users: ['user1'], user_id: 'user1' });
        ...
        it('should update a group', async () => {
            User.findById = jest.fn().mockResolvedValue({ _id: 'user1' });
            Group.findById = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Test Group', users: ['user1'], user_id: 'user1' });
            Group.findByIdAndUpdate = jest.fn().mockResolvedValue({ _id: 'group1', name: 'Updated Test Group', users: ['user1'] });
            ...
            expect(Group.findByIdAndUpdate).toHaveBeenCalledWith('group1', { name: 'Updated Test Group' }, { new: true });
            expect(res.status).toHaveBeenCalledWith(200);
            expect(res.json).toHaveBeenCalledWith(expect.anything());
        });    
    });    
